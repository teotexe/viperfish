...
void Attacks::init_pawn_attacks (bboard (&bb)[2][64]) {
    // White pawn attacks
...
void Attacks::init_knight_attacks (bboard (&bb)[64]) {
    for (square sq = a1; sq <= h8; sq++) {
...
void Attacks::init_king_attacks (bboard (&bb)[64]) {
    for (square sq = a1; sq <= h8; sq++) {
...
void Attacks::init_bishop_masks (bboard (&bb)[64]) {
    for (square sq = a1; sq <= h8; sq++) {
        int8_t rank = getrank(sq);
        int8_t file = getfile(sq);
...
void Attacks::init_rook_masks (bboard (&bb)[64]) {
    for (square sq = a1; sq <= h8; sq++) {
        int8_t rank = getrank(sq);
        int8_t file = getfile(sq);
...
void Attacks::init_bishop_attacks (bboard (&bb)[64][512]) {
    // Occupancy
    bboard occ;
...
void Attacks::init_rook_attacks (bboard (&bb)[64][4096]) {
    // Occupancy
    bboard occ;
